...
@Component({
  selector: 'lib-layout-license-device-code',
  imports: [InputTextModule, ReactiveFormsModule, MessageModule, CommonModule],
  templateUrl: './layout-license-device-code.html',
  styleUrl: './layout-license-device-code.scss',
})
export class LayoutLicenseDeviceCode implements OnInit, OnDestroy {
      ...
      .subscribe(value => {
        if (value.length == 5) {
          ...
          setTimeout(() => {
            ...
            const auth = `/${this.i18nRep.getLang()}/auth/license-device-success`;
