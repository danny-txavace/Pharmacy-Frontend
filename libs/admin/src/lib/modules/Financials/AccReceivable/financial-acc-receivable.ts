...
@Component({
  selector: 'lib-financial-acc-receivable',
  ...
  templateUrl: './financial-acc-receivable.html',
  styleUrl: './financial-acc-receivable.scss',
})
export class FinancialAccReceivable implements OnInit, OnDestroy {
  ...
  @ViewChild ('countUpQty_toReceive') countUpQty_toReceive!: ElementRef;
  @ViewChild ('countUpAmount_toReceive') countUpAmount_toReceive!: ElementRef;
  ...
  @ViewChild ('countUpQty_received') countUpQty_received!: ElementRef;
  @ViewChild ('countUpAmount_received') countUpAmount_received!: ElementRef;
  ...
  tableData() {
    ...
    [
      ...
      {
        headerName: this.translateService.instant('ADMIN.CENTRE.FINANCIAL.ACC_RECEIVABLE.TABLE.HEADER.NAME'),
        field: 'fullName', minWidth: 140, flex: 1, wrapText: true, autoHeight: true,
        cellClass: 'ag_cell_start',
      },
      {
        headerName: this.translateService.instant('ADMIN.CENTRE.FINANCIAL.ACC_RECEIVABLE.TABLE.HEADER.DESCRIPTION'),
        field: 'description', minWidth: 300, flex: 1, wrapText: true, autoHeight: true,
        cellClass: 'ag_cell_start'
      },
      {
        headerName: this.translateService.instant('ADMIN.CENTRE.FINANCIAL.ACC_RECEIVABLE.TABLE.HEADER.AMOUNT'),
  ...
  private loadingList(format?: string): void
  {
    ...
    /*this.subs.add(
      this.financialService.getTblAccountsReceivable().subscribe((data: IFinancialAccountsTblResponse[]) => {
        this.rowData = data;
  ...
  {
    /*this.subs.add(
      this.financialService.getCardsAccountsReceivable().subscribe((value: IAccountsReceivedCardResponse) => {
        setTimeout(() => {
          this.countUpRep.onCountUp('countUp-Qty', this.countUpQty_toReceive, value.toReceiveQty);
          this.countUpRep.onCountUp('countUp-Amount', this.countUpAmount_toReceive, value.toReceiveAmount);
          ...
          this.countUpRep.onCountUp('countUp-Qty', this.countUpQty_received, value.receivedQty);
          this.countUpRep.onCountUp('countUp-Amount', this.countUpAmount_received, value.receivedAmount);
        }, 300)
      })
    );*/
  }
